...
// ---------------------------------------------------------------------------
// Default seed data — unified sales-rep profile
// ---------------------------------------------------------------------------

const MG_DEFAULTS = {
  quoteNo: '220049173',
  date: '19.02.2024',
  billTo: 'Assc. Prof. Dr. Nurul Asma Abdullah\nHealth Campus\nUniversity Sains Malaysia\n16150 Kubang Kerian\nKelantan Darul Naim\nMalaysia',
...
const MGEN_DEFAULTS = {
  ...
  date: '15 Apr 2026',
  billTo: "Mr. Rocky Vester Anak Richmond\nUKM Specialist Children's Hospital (HPKK)\nJalan Yaakob Latif,\nBandar Tun Razak\n56000 Cheras\nMalaysia",
  phone: '+60 12 - 238 0658',
  email: '', // MGEN strictly omits email
  validity: '90 days',
  payment: '30 days',
  delivery: '06-08',
  salesperson: 'Dr. Ng Poh Kheng',
...
export function QuotationModule() {
  ...
  useEffect(() => {
    if (style === 'MGEN') {
      ...
      setMobile(MGEN_DEFAULTS.mobile);
  ...
  return (
    <div className="min-h-screen bg-slate-950 p-6 flex gap-6">
      ...
      <div className="w-1/3 bg-slate-900 border border-slate-700 rounded-xl p-6 overflow-y-auto h-[90vh] print:hidden">
        ...
        <div className="space-y-4">
          <div>
            ...
            <textarea className={`${inputClass} h-24`} value={billTo} onChange={(e) => setBillTo(e.target.value)} placeholder={style === 'MG' ? 'Bill To' : 'To'} />
          </div>
          {style === 'MG' && (
            <div>
          ...
          <div className="flex gap-2">
            <div className={style === 'MG' ? 'w-1/2' : 'w-full'}>
          ...
          {items.map((item, idx) => (
            <div key={item.id} className="bg-slate-800/60 border border-slate-700 rounded-lg p-3 space-y-2">
              <div className="flex justify-between items-center">
                ...
                <button onClick={() => removeItem(item.id)} className="text-[10px] font-bold text-red-400 hover:text-red-300 uppercase tracking-wide">
                  ✕ Delete
                </button>
              </div>
              <div>
                <FieldLabel>{style === 'MG' ? 'Cat No.' : 'Product No.'}</FieldLabel>
                <input className={smallInputClass} value={item.catNo} onChange={(e) => updateItem(item.id, 'catNo', e.target.value)} />
              </div>
              <div>
                <FieldLabel>Description</FieldLabel>
                <textarea className={`${smallInputClass} h-16`} value={item.desc} onChange={(e) => updateItem(item.id, 'desc', e.target.value)} />
              </div>
              <div className="flex gap-2">
                <div className="w-1/2">
                  <FieldLabel>Qty</FieldLabel>
                  <input type="number" min="0" step="1" className={smallInputClass} value={item.qty} onChange={(e) => updateItem(item.id, 'qty', parseFloat(e.target.value) || 0)} />
                </div>
                <div className="w-1/2">
                  <FieldLabel>Unit Price (MYR)</FieldLabel>
                  <input type="number" min="0" step="0.01" className={smallInputClass} value={item.price} onChange={(e) => updateItem(item.id, 'price', parseFloat(e.target.value) || 0)} />
                </div>
              </div>
            </div>
          ))}
          <button onClick={addItem} className="w-full border-2 border-dashed border-slate-600 hover:border-cyan-400 text-slate-400 hover:text-cyan-400 py-2.5 rounded-lg text-xs font-bold uppercase tracking-wide transition-colors">
            + Add Item
          </button>
        </div>
      ...
      <div className="flex-1 flex justify-center overflow-y-auto">
        <div
          className="bg-white text-black shadow-2xl border border-gray-300 w-[210mm] min-h-[297mm] relative"
          ...
          <div className="p-[10mm] text-xs font-sans">
            {style === 'MG' && (
              <div>
                ...
                <table className="w-full border-collapse border border-black mb-6">
                  <thead className="bg-gray-200">
                    <tr>
                      ...
                      <th className="border border-black p-2 w-[13%]">Unit Price<br />MYR</th>
                      <th className="border border-black p-2 w-[13%]">Total<br />MYR</th>
                    </tr>
                  </thead>
                  <tbody>
                    {items.map((it, idx) => (
                      <tr key={it.id} className="align-top">
                        ...
                        <td className="border-x border-black p-2"><DescriptionLines text={it.desc} /></td>
                    ...
                    {/* Spacer row with explicit borders on all cells */}
                    <tr className="h-16">
                    ...
                    {/* Subtotal / Total rows with full borders */}
                    <tr className="font-semibold">
                      <td colSpan={4} className="border border-black p-2 text-right">Subtotal</td>
                      <td colSpan={2} className="border border-black p-2 text-right">{formatCurrency(subtotal)}</td>
                    </tr>
                    <tr className="font-bold">
                      <td colSpan={4} className="border border-black p-2 text-right">Total</td>
                      <td colSpan={2} className="border border-black p-2 text-right">MYR {formatCurrency(subtotal)}</td>
                    </tr>
                  </tbody>
                </table>
                ...
                <div>
                  <p className="mb-4">
                    We hope the above package is favorable to you and are looking forward to your confirmed order.<br />
                    Thank You!
                  </p>
                  ...
                  <div className="text-[10px] text-center mt-12 pt-4 border-t border-gray-400">
                    51 Science Park Road, #04-16 The Aries, Singapore 117586 | Tel: 68739881 Fax: 68739897 | email: info@moleculargenomics.com.sg | website: www.moleculargenomics.com.sg<br />
                    RCB/ GST Reg No: 201101212G<br />
              ...
              <div>
                ...
                <div className="flex justify-between items-start mb-6">
                  ...
                  <div className="text-right text-[10px] font-bold leading-tight">
                    ...
                    Jalan Tun Abd Razak (Susur 1/1)<br />
                    Johor Bahru 80000, Malaysia<br />
                    Tel: (60) 7 2235620 Fax: (60) 7-2235620<br />
                ...
                <table className="w-full border-collapse border-y border-black mb-6">
                  <thead>
                    <tr className="border-b border-black">
                      ...
                      <th className="p-2 text-center w-[14%]">Unit Price<br />(RM$)</th>
                      <th className="p-2 text-center w-[14%]">Total Price<br />(RM$)</th>
                    </tr>
                  </thead>
                  <tbody>
                    {items.map((it, idx) => (
                      <tr key={it.id} className="align-top border-b border-gray-300">
                        <td className="p-2 text-center border-r border-gray-300">{idx + 1}</td>
                        <td className="p-2 border-r border-gray-300">{it.catNo}</td>
                        <td className="p-2 border-r border-gray-300"><DescriptionLines text={it.desc} /></td>
                        <td className="p-2 text-center border-r border-gray-300">{it.qty}</td>
                        <td className="p-2 text-center border-r border-gray-300">RM$<br />{formatCurrency(it.price)}</td>
                        <td className="p-2 text-center">RM$<br />{formatCurrency(it.qty * it.price)}</td>
                      </tr>
                    ))}
                    <tr className="font-bold">
                      <td colSpan={5} className="p-2 text-right">TOTAL</td>
                      <td className="p-2 text-center">RM$<br />{formatCurrency(subtotal)}</td>
                    </tr>
                  </tbody>
                </table>
                ...
                <div className="flex justify-between items-end gap-6">
                  <div className="w-1/2">
                    ...
                    <p className="mb-2">Looking forward to your order.<br /><br />Thank You!</p>
                    <p>Yours truly,<br /><br />{salesperson}</p>
                  </div>

                  <div className="w-[45%] border border-black mb-4">
